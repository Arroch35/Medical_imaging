...
class TripletDataset(data.Dataset):
    ...
    def __init__(self, latents, labels):
        ...
        # Safety check: every class must have at least 2 samples
        for y, idxs in self.class_indices.items():
            if len(idxs) < 2:
                raise ValueError(f"Class {y} has < 2 samples. Cannot form triplets.")

        self.labels_unique = list(self.class_indices.keys())

    def __len__(self):
        # We return N triplets per epoch
        return self.N
    ...
    def __getitem__(self, index):
        ...
        # ----- Negative -----
        neg_label = random.choice([c for c in self.labels_unique if c != anchor_label])
        neg_idx = random.choice(self.class_indices[neg_label])

        return (
            self.latents[anchor_idx],
            self.latents[pos_idx],
            self.latents[neg_idx]
        )
